...
async def scrape_track(url: str):
...
async def scrape_spotify(query: str) -> Union[str, List[str]]:
    async with ClientSession() as session:
        async with session.get(query) as response:
            ...
            try:
                ...
                pattern = r'<meta\s+name="music:song"\s+content="([^"]+)"\s*/?>'

                # Find all matches in the HTML content
                matches = set(re.findall(pattern, data))
                return await asyncio.gather(*[scrape_track(url) for url in matches])
...
class MusicCommands(Cog):
        ...
        if query:
            ...
            if "open.spotify.com" in query:
                ...
                if isinstance(query, list):
                    ...
                    async def add(q: str):
                        nonlocal added
                        with suppress(LavalinkLoadException, IndexError):
                            ...
                            if track:
                                ...
                                added += 1
                    [await add(q) for q in query]
                    return await ctx.success(f"added **{added}** items from the playlist supplied to the queue")
    ...
    async def queue(self, ctx: Context):
        ...
        if len(tracks) == 0:
            raise CommandError("there are no tracks playing")

        return await ctx.paginate(
            Embed(
                title="Queue",
            ).set_footer(text = f"{len(tracks)} {pluralize('track', len(tracks))} • {format_duration(sum(track.length for track in tracks))}"),
            [
                f"**{index + 1}.** [{track.title}]({track.uri}) by **{track.author}** {f'[{str(requester)}]'}"
                for index, track in enumerate(tracks)
                if (
                    requester := ctx.guild.get_member(
                        getattr(track.extras, "requester_id", ctx.author.id)
                    )
                )
            ],
        )
