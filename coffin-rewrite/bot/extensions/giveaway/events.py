...
class GiveawayEvents(Cog):
    ...
    async def giveaway_check(self):
        async with self.locks["giveaway"]:
            try:
                for giveaway in await self.bot.db.fetch("""SELECT * FROM giveaways""", cached = False):
                    ...
                    if giveaway.expiration <= utils.utcnow():
                        ...
                        if channel:
                            ...
                            logger.info(f"{giveaway.expiration.timestamp()} - {utils.utcnow().timestamp()}")
    ...
    async def giveaway_ended(self, guild: Guild, channel: TextChannel, giveaway: Record):
        ...
        embed = Embed(title = "Giveaway Ended", description = f"The giveaway has ended.\n**Winners:** {winners_string}\n**Hosted By:** {hosts_string}")
        win_message = await message.reply(embed = embed)
        await self.bot.db.execute("""UPDATE giveaways SET win_message_id = $1 WHERE message_id = $2""", win_message.id, giveaway.message_id)
    ...
    async def on_giveaway_enter(self, payload: RawReactionActionEvent):
        ...
        if member.bot:
            return 
        if not (entries := await self.bot.db.fetchval("""SELECT entries FROM giveaways WHERE message_id = $1""", payload.message_id, cached = False)):
            entries = []
        if member.id in entries:
            return 
        entries.append(member.id)
        await self.bot.db.execute("""UPDATE giveaways SET entries = $1 WHERE message_id = $2""", entries, payload.message_id)
    ...
    async def on_giveaway_leave(self, payload: RawReactionActionEvent):
        ...
        if member.bot:
            return
        if not (entries := await self.bot.db.fetchval("""SELECT entries FROM giveaways WHERE message_id = $1""", payload.message_id)):
            return
