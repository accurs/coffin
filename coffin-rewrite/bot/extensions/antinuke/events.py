...
from bot.system.classes.builtins import catch
from .model import Module, Configuration
...
class AntiNukeEvents(Cog):
    ...
    async def _do_punishment(self, entry: AuditLogEntry, module: Module, configuration: Configuration, bypass: Optional[bool] = False):
    ...
    async def on_audit_log_entry_role_update(self, entry: AuditLogEntry):
        ...
        if not data:
            ...
            return
        try:
            await entry.target.edit(**{k:v for k, v in entry.changes.before.__iter__()})
        except Exception:
            pass
        return await self.do_punishment(entry, data.role, data)
    ...
    async def on_audit_log_entry_channel_delete(self, entry: AuditLogEntry):
        ...
        if overwrites := kwargs.pop("overwrites"):
            overwrites = {self.get_object(entry.guild, k) if isinstance(k, Object) else k: v for k, v in overwrites}
    ...
    async def on_audit_log_entry_channel_update(self, entry: AuditLogEntry):
        ...
        if not data:
            ...
            return
        kwargs = {k:v for k, v in entry.before.__iter__() if k not in ("flags", "type")}
        if overwrites := kwargs.pop("overwrites"):
            overwrites = {self.get_object(entry.guild, k) if isinstance(k, Object) else k: v for k, v in overwrites}
            kwargs["overwrites"] = overwrites
        try:
