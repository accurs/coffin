...
def is_donator():
  async def predicate(ctx: Context):
    ...
    if not result and ctx.author.id not in ctx.bot.owner_ids:
      raise commands.BadArgument(
        f"This command is for donators only! Please join our [**support server**]({COFFIN.SUPPORT_URL}) for more information"
      )
    return True
  return commands.check(predicate)
