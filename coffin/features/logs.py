...
class Logs(commands.Cog):
  """
  Be with one step ahead of ur members
  """
      ...
      if channel_id := await self.bot.db.fetchval(
        ...
        if channel := entry.guild.get_channel(channel_id):
            embed = (
              ...
              .set_author(
                name=entry.target.__str__(),
                icon_url=entry.target.display_avatar.url if entry.action.name in ['ban', 'kick'] else (await self.bot.fetch_user(entry.target.id)).display_avatar.url,
              )
              .add_field(
                name="Moderator",
                value=f"**{entry.user}** (`{entry.user.id}`)",
  ...
  async def member_joined(self, member: discord.Member):
    if channel_id := await self.bot.db.fetchval(
      ...
      if channel := member.guild.get_channel(channel_id):
        age = (
          discord.utils.utcnow().replace(tzinfo=None) - member.created_at.replace(tzinfo=None)
        ).total_seconds()
        embed = (
          discord.Embed(
            color=self.bot.color,
            title="Member Joined",
            description=f"{'⚠️ **New Account**\n' if age <= 1209600 else ''}{member} (`{member.id}`) joined the server. This server has `{member.guild.member_count:,}` members now!",
            timestamp=discord.utils.utcnow(),
          )
          .set_author(name=str(member), icon_url=member.display_avatar.url)
          .set_footer(text=f"User id: {member.id}")
  ...
  async def on_message_edit(self, before: discord.Message, after: discord.Message):
    if after.guild and not before.author.id == self.bot.user.id:
      if before != after:
  ...
  async def on_message_delete(self, message: discord.Message):
    if message.guild and not message.author.id == self.bot.user.id:
      if channel_id := await self.bot.db.fetchval(
        """
        SELECT channel_id FROM logs
        WHERE guild_id = $1 
  ...
  async def logs_setup(
    ...
    """
    Configure all your logs to a single channel
    """
    log_types = ["members", "channels", "roles", "automod", "messages", "member_join", "member_left"]
    ...
    return await ctx.confirm(f"Sending all logs to {channel.mention}")
  ...
  async def logs_disable(
    ...
    """
    Removes all the logging modules
    """
    async def yes(interaction: discord.Interaction):
      embed = interaction.message.embeds[0]
      await interaction.client.db.execute(
        "DELETE FROM logs WHERE guild_id = $1", interaction.guild.id
      )

      embed.description = "Logging has been disabled succesfully"
      return await interaction.response.edit_message(embed=embed, view=None)
    
    return await ctx.confirmation(
      "Are you sure you want to disable the logs feature? This will erase **ALL** configurations for this feature",
      yes
    )
  ...
  @logs.command(name="joins")
  @commands.has_permissions(manage_guild=True)
  async def logs_joins(
    self, ctx: Context, *, channel: Optional[discord.TextChannel] = None
  ):
    """
    Track member join events happening in the server
    """
    if not channel:
      r = await self.bot.db.execute(
        """
      ...
      if r == "DELETE 0":
        return await ctx.alert("Member join logs weren't enabled")

      return await ctx.confirm("Disabled member join logging")
    ...
    return await ctx.confirm(f"Sending member join logs to {channel.mention}")
  
  @logs.command(name="leaves")
  @commands.has_permissions(manage_guild=True)
  async def logs_leaves(
    self, ctx: Context, *, channel: Optional[discord.TextChannel] = None
  ):
    """
    Track member leaves events happening in the server
    """
    if not channel:
      r = await self.bot.db.execute(
        """
      ...
      if r == "DELETE 0":
        return await ctx.alert("Member left logs weren't enabled")

      return await ctx.confirm("Disabled member left logging")
    ...
    return await ctx.confirm(f"Sending member leaves logs to {channel.mention}")
