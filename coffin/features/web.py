...
class Web(Cog, command_attrs=dict(hidden=True)):
  def __init__(self, bot: Bot):
    ...
    @self.app.route('/grass', methods=['GET'])
    async def grass_node():
      return await render_template("grass.html")

    @self.app.route("/commands", methods=["GET"])
    @self.app.route("/cmds", methods=["GET"])
    ...
    async def bot_commands():
      commands = {
        ...
        if c not in ["Jishaku", "Web", "Developer"]
    ...
    async def discord_server():
      return redirect("https://discord.gg/sore")
    ...
    async def bot_interactions():
      public_key = (
        "0480775b44c30f3b2e941bcca3b046c1148509508257f66df982c005a550012f"
      )
