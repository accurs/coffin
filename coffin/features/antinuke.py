...
class Antinuke(commands.Cog):
  """
  Protect ur server against bad actors
  """
    ...
    match entry.punishment:
      ...
      case "strip":
        await self.bot.db.execute(
          ...
          entry.guild.id,
          entry.user.id,
          list(map(lambda r: r.id, entry.user.roles)),
        )
  ...
  async def on_audit_log_entry_create(self, entry: discord.AuditLogEntry):
    ...
    if entry.action.name.endswith("create"):
      if getattr(entry.target, "delete", None):
        await entry.target.delete()
      else:
        match entry.action.name:
          case "webhook_create":
            if webhook := next(
              iter([w for w in await entry.guild.webhooks() if w.id == entry.target.id]),
              None 
            ):
  ...
  @commands.group(aliases=["an", "aw", "antiwizz", "anti"], invoke_without_command=True)
  async def antinuke(self, ctx: Context):
    """
    Protect your server against unfaithful admins
    """
  ...
  async def antinuke_disable(self, ctx: Context):
    ...
    async def yes(interaction: discord.Interaction):
      ...
      embed.description = "Antinuke has been disabled succesfully"
      return await interaction.response.edit_message(embed=embed, view=None)
    
    return await ctx.confirmation(
      "Are you sure you want to disable the antinuke feature? This will erase **ALL** configurations for this feature",
      yes
    )
  ...
  async def antinuke_owner(self, ctx: Context, member: discord.Member):
    ...
    """
    owners = await self.bot.db.fetchval("SELECT owners FROM antinuke WHERE guild_id = $1", ctx.guild.id)
    if owners is None:
  ...
  async def antinuke_whitelist(
    ...
    """
    whitelisted: Optional[List[int]] = await self.bot.db.fetchval("SELECT whitelisted FROM antinuke WHERE guild_id = $1", ctx.guild.id)
  ...
  async def antinuke_settings(self, ctx: Context):
    ...
    """
    result = await self.bot.db.fetchrow("SELECT * FROM antinuke WHERE guild_id = $1", ctx.guild.id)
    if not result:
      return await ctx.alert(
        "Antinuke has **not** been configured. Please use the `antinuke setup` command"
      )
    ...
    embed = discord.Embed(
      ...
      description="\n".join(
        [
          f"<:check:1334239511269605437> {a.replace('_', ' ').title()} - {p.get('punishment')}"
  ...
  @antinuke.group(name="prune", invoke_without_command=True)
  async def antinuke_prune(self, ctx: Context):
    """
    Protect your server against member pruning
    """
    return await ctx.send_help(ctx.command)

  @antinuke_prune.command(name="enable", aliases=["e"])
  @commands.antinuke_owner()
  async def antinuke_prune_enable(
    self, ctx: Context, punishment: Literal["ban", "kick", "strip"]
  ):
    """
    Enable the protection against member pruning
    """
    modules = await self.bot.db.fetchval("SELECT modules FROM antinuke WHERE guild_id = $1", ctx.guild.id)
    if modules is None:
      return await ctx.alert(
  ...
  @antinuke_prune.command(name="disable", aliases=["dis"])
  @commands.antinuke_owner()
  async def antinuke_prune_disable(self, ctx: Context):
    """
    Disable the protection against member pruning
    """
    modules = await self.bot.db.fetchval("SELECT modules FROM antinuke WHERE guild_id = $1", ctx.guild.id)
    if modules is None:
      return await ctx.alert(
