...
class Blackjack(discord.ui.View):
  def __init__(self, author_id: int, bet: int):
    ...
    self.win = "🏆"
    self.lose = "😔"
    self.deck: list[str] = [f"{s} {n}" for s in ["♣", "♦", "♥", "♠"] for n in range(1, 11)]
    super().__init__()
  ...
  async def on_timeout(self):
    if not self.ended:
      ...
      self.bot.blackjack_matches.remove(self.author_id)
...
class Economy(commands.Cog):
  ...
  async def cog_check(self, ctx: Context):
    ...
    if (
      ...
      and ctx.command.qualified_name not in ["balance", "daily"]
    ):
  ...
  async def blackjack(self: "Economy", ctx: Context, bet: Annotated[int, Value]):
    ...
    embed.set_thumbnail(url="https://coffin.lol/static/cards.png")
    embed.add_field(
  ...
  async def monthly(self: "Economy", ctx: Context):
    ...
    async with self.locks[ctx.author.id]:
      ...
      donator = await self.bot.db.fetchval(
        "SELECT user_id FROM donator WHERE user_id = $1", ctx.author.id
      )
      ...
      return await ctx.confirm(
        f"You have collected `{points}` credits. Come back next month. (+20% more)"
        if donator else
        f"You have collected `{points}` credits. Come back next month"
      )
  ...
  async def daily(self: "Economy", ctx: Context):
    ...
    async with self.locks[ctx.author.id]:
      ...
      return await ctx.confirm(
        f"You have collected `{points}` credits. Come back tomorrow (+20% more)"
        if donator else
        f"You have collected `{points}` credits. Come back tomorrow"
      )
  ...
  async def work(self: "Economy", ctx: Context):
    ...
    amount = secrets.choice(list(range(1, 3000)))
    ...
    return await ctx.neutral(
      (
        ...
        f"**{job}** and earned {f'**{amount:,}** credits (+20%)' if donator else f'**{amount:,}** credits'}"
      )
    )
