...
class LastFM(Cog):
  """
  This is lastfm a service ran by some indians
  """
  ...
  async def on_message(self, message: Message):
    if cmd := await self.bot.db.fetchval(
      ...
      if message.content == cmd:
        if not ratelimiter(
          bucket=f"lf-{message.channel.id}", key="lf", rate=2, per=3
        ):
          if not await self.bot.db.fetchval(
            "SELECT * FROM disabledcmds WHERE guild_id = $1 AND command_name = $2",
            message.guild.id,
            "nowplaying"
          ):
    ...
    *, 
    ...
    if not (
      ...
      return await ctx.alert(
        f">>> You don't have a LastFM username set!\nUse [`{ctx.clean_prefix}lastfm set <username>`](https://last.fm/join) to connect it."
  ...
  async def lf_spotify(
    self, 
    ctx: Context,
    *,
    member: Optional[User] = Author
  ):
    """
    Get your recent scrobbled song as a spotify url
    """
    if not (
      result := await self.bot.db.fetchrow(
        """
    ...
    pl = await self.bot.session.post("https://accounts.spotify.com/api/token", headers=hdrs, data=form)
    access_token = pl['access_token']
    ...
    params = {
      "q": f"{data.track.name} {data.artist.name}",
      "type": "track",
      "limit": 1
    }

    payload = await self.bot.session.get(url, headers=headers, params=params)
    return await ctx.send(payload['tracks']['items'][0]['external_urls']['spotify'])
  ...
  async def lastfm_globalwhoknows(
    ...
    """
    Get the top listeners of a specific artist from all servers that have coffin
    """
    if not (
  ...
  @lastfm.command(name="reactions", example="lastfm reactions :skull:\nlastfm reactions remove\nlastfm reactions default")
  async def lastfm_reactions(
    ...
    *,
    ...
    """
    if reactions.lower() == "remove":
      await self.bot.db.execute(
        "UPDATE lastfm.user SET reactions = $1 WHERE user_id = $2",
        [], ctx.author.id
      )
      return await ctx.confirm("Removed your lastfm reactions")
    elif reactions.lower() == "default":
      await self.bot.db.execute(
        "UPDATE lastfm.user SET reactions = $1 WHERE user_id = $2",
        ["🔥", "🗑️"], ctx.author.id
      )
      return await ctx.confirm("Updated your lastfm reactions to **default**")
    else:
      emojis = [
      ...
      await self.bot.db.execute(
        "UPDATE lastfm.user SET reactions = $1 WHERE user_id = $2",
        emojis, ctx.author.id
      )
      return await ctx.confirm(f"Updated your lastfm reactions to {' '.join(emojis)}")
  ...
  async def lastfm_collage(self, ctx: Context):
    """
    Commands for the collage group
    """
    return await ctx.send_help(ctx.command)
