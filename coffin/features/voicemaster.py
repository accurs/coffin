...
def is_vc_owner():
  async def predicate(ctx: Context):
    ...
    results = await ctx.bot.db.fetchval("SELECT voice_channels FROM voicemaster WHERE guild_id = $1", ctx.guild.id)
    if not results:
...
class VoiceMaster(commands.Cog):
  ...
  async def build_interface(self: "VoiceMaster", ctx: Context):
    channel_id = await self.bot.db.fetchval("SELECT channel_id FROM voicemaster WHERE guild_id = $1", ctx.guild.id)
  ...
  async def voicemaster_setup(self: "VoiceMaster", ctx: Context):
    ...
    if result:
      return await ctx.alert("The VoiceMaster feature is **already** configured")
