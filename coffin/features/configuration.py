...
class Configuration(Cog):
  ...
  async def transcribe(self, attachment: Attachment):
    ...
    with suppress(Exception):
      ...
      segments, _ = model.transcribe(audio, word_timestamps=False, vad_filter=False)
      del model
      gc.collect()
    ...
    if message.guild:
      if result := await self.bot.db.fetchrow(
        ...
        if (
          ...
          and "Bump done! :thumbsup:" in message.embeds[0].description
        ):
          user = message.interaction_metadata.user
          bump_next = discord.utils.utcnow() + datetime.timedelta(hours=2)
          code = await self.bot.embed.convert(
            user,
            result['thank'].replace("{bump_timestamp}", str(int(bump_next.timestamp())))
          )
  ...
  async def on_image_only(self, message: discord.Message):
    if message.guild:
      if not message.attachments: 
        if await self.bot.db.fetchrow(
          ...
          delay = 5 if ratelimiter(bucket="imgonly", key=f"{message.channel.id}", rate=2, per=5) else None
      ...
      if isinstance(message.author, discord.Member):
        if (
          ...
          if result := await self.bot.db.fetchrow(
            ...
            if message.author.id not in result.whitelisted:
  ...
  async def whitelist_protect(self: "Configuration", member: Member):
    ...
    if result:
      ...
      if member.id not in result.whitelisted:
  ...
  async def giveaway_list(self, ctx: Context):
      ...
      """
      return await ctx.send(f"[**{ctx.guild} giveaways**](https://coffin.lol/giveaways/{ctx.guild.id})")
  ...
  async def giveaway_end(self, ctx: Context, message_id: int):
      ...
      try:
          ...
          try:
              message = await self.bot.get_channel(gw.channel_id).fetch_message(message_id)
              ...
              return await ctx.confirm("Ended giveaway")
  ...
  async def giveaway_create(
    ...
    giveaways = await self.bot.db.fetchval(
      "SELECT COUNT(*) FROM giveaway WHERE guild_id = $1 AND ended = $2", ctx.guild.id, False
    )
    if giveaways == 5:
      return await ctx.alert(
  ...
  async def giveaway_edit(self: "Configuration", ctx: Context):
    """
    Edit the giveaway
    """
    return await ctx.send_help(ctx.command)
  ...
  async def giveaway_edit_prize(
    ...
    *,
    ...
    """
    Edit the giveaway prize
    """
    if not self.bot.giveaways.get(message.id):
      return await ctx.alert("Could not find giveaway")
    ...
    await self.bot.db.execute(
      "UPDATE giveaway SET reward = $1 WHERE message_id = $2",
      prize,
      message.id
    )
    return await ctx.confirm(f"Giveaway prize edited to `{prize}`")
  ...
  async def giveaway_edit_host(
    ...
    *,
    ...
    data = [
      ...
      f"Ending: {discord.utils.format_dt(gw.ending, style='R')}",
      f"Winners: **{gw.winners}**",
    ]
    embed.description = "\n".join(data)
    await message.edit(embed=embed)
    await self.bot.db.execute(
      "UPDATE giveaway SET host = $1 WHERE message_id = $2",
      host.id,
      message.id
    )
    return await ctx.confirm(f"Giveaway host edited to {host.mention}")
  ...
  async def giveaway_edit_winners(
    ...
    *,
    ...
    await message.edit(embed=embed)
    await self.bot.db.execute(
      "UPDATE giveaway SET winners = $1 WHERE message_id = $2",
      count,
      message.id
    )
    return await ctx.confirm(f"Giveaway winners edited to `{count}`")
  ...
  async def giveaway_edit_duration(
    ...
    *,
    ...
    embed = message.embeds[0]
    ending = datetime.datetime.now() + datetime.timedelta(seconds=int(duration))
    data = [
      f"Hosted by: <@{gw.host}>",
      f"Ending: {discord.utils.format_dt(ending, style='R')}",
      f"Winners: **{gw.winners}**",
    ]
    embed.description = "\n".join(data)
    await message.edit(embed=embed)
    await self.bot.db.execute(
      "UPDATE giveaway SET ending = $1 WHERE message_id = $2",
      ending,
      message.id
    )
    return await ctx.confirm(f"Giveaway duration edited to `{humanfriendly.format_timespan(duration)}`")
  ...
  async def boosterrole_create(self, ctx: Context, *, name: Optional[str] = None):
    ...
    role = await ctx.guild.create_role(name=name or f"{ctx.author.name}'s role", reason="Booster role created")
  ...
  async def boosterrole_display(self, ctx: Context, display: bool):
    """
    Display your booster role separately
    """
    if not (
      role_id := await self.bot.db.fetchval(
        """
    ...
    await role.edit(hoist=display)
    return await ctx.confirm(f"Edited the role's display to `{display}`")
  ...
  async def boosterrole_delete(self, ctx: Context):
    ...
    await self.bot.db.execute(
     "DELETE FROM boosterrole.share WHERE role_id = $1", role.id 
    )

    return await ctx.confirm("Deleted your custom role")
  
  @boosterrole.command(name="share", invoke_without_command=True)
  async def boosterrole_share(self, ctx: Context, *, member: Member):
    """
    Share ur custom role with others
    """
    if not (
      role_id := await self.bot.db.fetchval(
        """
    ...
    if role in member.roles:
      ...
      return await ctx.confirm(f"Stopped sharing ur booster role with {member.mention}")
    else:
      await member.add_roles(role)
      return await ctx.confirm(f"Shared ur booster role with {member.mention}")
  ...
  @settings.command(name="disabledcommands", aliases=["disabledcmds"])
  @has_permissions(administrator=True)
  async def settings_disabledcommands(self, ctx: Context):
    """
    Get a list with the disabled commands in the server
    """
    if not (
      results := await self.bot.db.fetch(
        "SELECT * FROM disabledcmds WHERE guild_id = $1", ctx.guild.id
      )
    ):
      return await ctx.alert("There are no disabled commands in this server")
    
    return await ctx.paginate(
      [
        f"`{r.command_name}` has been disabled {discord.utils.format_dt(r.date, style='R')}" for r in results
      ],
      Embed(title=f"Disabled commands in {ctx.guild.name}")
    )
  ...
  async def autorole_list(self: "Configuration", ctx: Context):
    ...
    """
    roles = await self.bot.db.fetchval("SELECT roles FROM autorole WHERE guild_id = $1", ctx.guild.id)
    if not roles:
  ...
  async def whitelist_dm(self: "Configuration", ctx: Context, *, message: str):
    ...
    """
    r = await self.bot.db.execute("UPDATE whitelist SET msg = $1 WHERE guild_id = $2", message, ctx.guild.id)
    if r == "UPDATE 0":
      return await ctx.alert(
  ...
  async def antispam_disable(self, ctx: Context):
    ...
    """
    r = await self.bot.db.execute("DELETE FROM antispam WHERE guild_id = $1", ctx.guild.id)
    if r == "DELETE 0":
  ...
  async def antispam_whitelist(
    ...
    whitelisted = await self.bot.db.fetchval("SELECT whitelisted FROM antispam WHERE guild_id = $1", ctx.guild.id)
  ...
  @filter_invites.command(name="enable", aliases=["e"], example="block")
  @has_permissions(manage_guild=True)
  async def filter_invites_enable(
    ...
    try:
      ...
      if e.response.status_code == 400:
        return await ctx.alert("This server cannot have any more automod rules of this kind")
    except Exception as e:
      print(e)
      return await ctx.alert("Unknown error occured")
  ...
  async def guildedit_name(self: "Configuration", ctx: Context, *, name: str):
    ...
    """
    await ctx.guild.edit(name=name, reason=f"Guild {ctx.command.name} edited by {ctx.author.name}")
    return await ctx.confirm(
  ...
  async def aliases_list(self: "Configuration", ctx: Context):
    ...
    """
    aliases = await self.bot.db.fetch("SELECT * FROM aliases WHERE guild_id = $1", ctx.guild.id)
    if not aliases:
  ...
  async def aliases_remove(self: "Configuration", ctx: Context, alias: str):
    ...
    if r == "DELETE 0":
      return await ctx.alert("This is **not** a valid alias")
  ...
  async def ticket_disable(self: "Configuration", ctx: Context):
    ...
    """
    r = await self.bot.db.execute("DELETE FROM tickets WHERE guild_id = $1", ctx.guild.id)
    if r == "DELETE 0":
  ...
  async def ticket_settings(self: "Configuration", ctx: Context):
    ...
    embed = (
      ...
      .set_footer(text="coffin.lol", icon_url=self.bot.user.display_avatar.url)
    )
  ...
  async def ticket_send(
    ...
    *,
    ...
    if not (
      ...
      panel = await self.bot.db.fetchval(
        """
        INSERT INTO tickets (guild_id) VALUES ($1)  
        RETURNING panel_message
        """,
        ctx.guild.id
      )
  ...
  async def ticket_category(
    ...
    ctx: Context,
    *,
    category: Union[CategoryChannel, Literal["none"]],
  ):
    """
    ...
    return await ctx.confirm(
      f"Assigned your category to #{category}"
      if category_id
  ...
  async def ticket_topic_add(self: "Configuration", ctx: Context, *, topic: str):
    ...
    Add a ticket topic
    """
    topics = await self.bot.db.fetchval("SELECT topics FROM tickets WHERE guild_id = $1", ctx.guild.id)
    if topics is None:
      topics = []
  ...
  async def bumpreminder_disable(self: "Configuration", ctx: Context):
    ...
    """
    r = await self.bot.db.execute("DELETE FROM bumpreminder WHERE guild_id = $1", ctx.guild.id)
    if r == "DELETE 0":
  ...
  async def imageonly(self, ctx: Context):
    """
    Delete messages that do not contain attachments
    """
    return await ctx.send_help(ctx.command)
  
  @imageonly.command(name="add")
  @has_permissions(manage_guild=True)
  async def imgonly_add(
    self, 
    ctx: Context, 
    *, 
    channel: TextChannel
    ...
    """
    Assign a channel as image only
    """
    r = await self.bot.db.execute(
      """
      INSERT INTO imgonly VALUES ($1,$2)
    ...
    if r == "INSERT 0":
      return await ctx.alert("This channel is **already** assigned as image only")
    
    return await ctx.confirm(f"Assigned {channel.mention} as image only")
  
  @imageonly.command(name="remove", aliases=['rm'])
  @has_permissions(manage_guild=True)
  async def imgonly_remove(
    self,
    ctx: Context,
    *,
    channel: TextChannel
    ...
    """
    Unassign a channel as image only
    """
    r = await self.bot.db.execute(
      """
      DELETE FROM imgonly 
    ...
    if r == "DELETE 0":
      return await ctx.alert("This channel was not assigned as image only")
    
    return await ctx.confirm(f"Unassigned {channel.mention} as image only")
  
  @imageonly.command(name="clear")
  @has_permissions(administrator=True)
  async def imgonly_clear(
    self,
    ctx: Context
  ):
    """
    Instantly remove all image only channels from this server
    """
    if not await self.bot.db.fetch("SELECT * FROM imgonly WHERE guild_id = $1", ctx.guild.id):
      return await ctx.alert(f"There are **no** image only channels in **{ctx.guild}**")
    
    async def yes(interaction: discord.Interaction):
      await interaction.client.db.execute(
        "DELETE * FROM imgonly WHERE guild_id = $1",
        interaction.guild.id
      )

      embed = interaction.message.embeds[0]
      embed.description = "Removed all image only channels"
      return await interaction.response.edit_message(embed=embed, view=None)
    
    return await ctx.confirmation(
      "Are you sure you want to disable the image only module?", yes
    )
  
  @imageonly.command(name="channels")
  @has_permissions(manage_guild=True)
  async def imgonly_channels(
    self,
    ctx: Context
  ):
    """
    View all image only channels from this server
    """
    results = await self.bot.db.fetch(
      "SELECT * FROM imgonly WHERE guild_id = $1",
      ctx.guild.id
    )
    if not results:
      return await ctx.alert("There are **no** image only channels in this server")
    
    return await ctx.paginate(
      [
        f"<#{result.channel_id}> (`{result.channel_id}`)"
        for result in results
      ],
      Embed(title=f"Image only channels in {ctx.guild}")
    )
